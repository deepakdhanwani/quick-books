...
public class SaleService {
    ...
    public PageResponse<SaleResponse> findPage(Long subscriberId, int page, int size, String search) {
        ...
        Page<SaleResponse> result = saleRepository.findBySubscriber(subscriberId, normalizedSearch, pageable)
    ...
    public SaleResponse receivePayment(Long subscriberId,
        ...
        if (paymentAmount.compareTo(sale.getPendingAmount()) > 0) {
        ...
        BigDecimal newPendingAmount = sale.getTotalAmount().subtract(newPaidAmount).max(BigDecimal.ZERO);
