...
|--------|-----------|-------|
| **Admin** | id, email, password_hash, name, created_at | Platform administrator; logs in via web portal. Created once from `ADMIN_EMAIL` / `ADMIN_PASSWORD` in `.env`. |
...
| User | Method | Details |
|------|--------|---------|
| **Admin** | Email + password | Admin email and password are defined in `backend/.env` (`ADMIN_EMAIL`, `ADMIN_PASSWORD`). On first startup only, Spring Boot creates the admin account from these properties. |
...
- Admin credentials from `.env`, seeded once on first startup
